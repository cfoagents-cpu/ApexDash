...
import { Mail, MessageSquare, Phone, Link2, Plus, Trash2, LogOut } from 'lucide-react';
...
export default function OutreachPage() {
  ...
  useEffect(() => {
    const token = localStorage.getItem('fm_private_token');
    if (!token) { router.push('/private/login'); return; }
    const saved = localStorage.getItem(STORAGE_KEY);
    if (saved) setData(JSON.parse(saved));
  }, [router]);

  function logout() {
    localStorage.removeItem('fm_private_token');
